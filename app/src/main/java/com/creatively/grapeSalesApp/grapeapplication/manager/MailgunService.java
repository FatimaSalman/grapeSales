...
public class MailgunService {
    private final static String MAILGUN_API_USERNAME = "api";

    private final HttpUrl mailgunApiUrl;
    private final String authHeaderValue;

    private MailgunService(HttpUrl mailgunbaseUrl, String mailgunDomain, String mailgunApiKey) {
        this.mailgunApiUrl = HttpUrl.parse(mailgunbaseUrl.toString() + "/" + mailgunDomain);
        this.authHeaderValue = buildAuthHeader(mailgunApiKey);
    }

    private boolean sendMail(MailgunEmail email) throws IOException {
    ...
    public static void main(String[] args) throws IOException {
        MailgunService mailgunService = new MailgunService(
                Objects.requireNonNull(HttpUrl.parse("https://api.mailgun.net/v3")),
